...
当前尚无线上部署地址，因此未配置 `WEB_BASE_URL` 时直接加载内置前端。生产构建允许省略
`WEB_BASE_URL` 以发布内置版；一旦配置远程前端地址则必须使用 HTTPS。生产构建必须配置
HTTPS `API_BASE_URL`。HTTP 只通过 debug 平台配置为明确的本地开发地址放行。

内置前端不能把 `API_BASE_URL` 只编译进静态 JavaScript。回环服务需要提供运行时配置脚本，
前端启动时优先读取其中的 API 地址，再回退到 Vite 构建变量。这样同一份内置产物可以安全地
使用 Flutter 构建参数指定的开发、测试或生产后端。
...
发送到回环服务。同步脚本负责生成不含环境秘密的默认配置；App 启动后由回环服务基于
`--dart-define` 值生成本次运行的配置响应。API 地址不是密钥，真正的服务凭据不得放入该配置。
...
注销状态重新建立应用会话。若外部认证使用稳定的 provider subject，则另存不含个人资料的注销
身份墓碑；该 subject 不得自动恢复，但不阻止原邮箱作为新的本地账号重新注册。
...
- 配置解析、运行时 API 注入、URL 白名单、导航决策和降级状态机单元测试；
- Bridge 来源、命令和参数验证测试；
